...
The proof of this theorem is quite involved, and is presented in full detail in \cite{gabbay1994}. In the next few sections, we'll give a high-level overview of Gabbay et. al.'s scheme. To mirror their notation, we'll write $U$ formulas as $U(p, q)$ instead of $q \,\mathcal{U}\, p$.
...
The reason we cannot continue the arguments we presented in the previous section and separate mixed $\Darrow$ formulas is the same reason behind Marx's mistake; the fact that the $\Darrow$ connective isn't deterministic in its choice of the downward path. The argument we earlier presented in \cref{fig:darrow-a-b-and-c} implies that one cannot write each $\Darrow$ formula in the form
\begin{equation*}
...
\end{equation*}
There are bigger consequences of this non-determinism. $\Darrow$ formulas that appear in the path condition of other $\Darrow$ formulas don't necessarily traverse the same path as their parent, as made evident in \cref{darrow-unreliable}.
\begin{figure}[h]
    \centering
    \tikzfig{darrow-unreliable}
    \caption[]{\emph{The unreliability of $\Darrow$.} The present point must satisfy $\Darrow(a, b \lor \Darrow(c, d))$. No point along the path to $a$ is a $c$.}
    \label{fig:linear-separation-regions}
\end{figure}
...
In \cite{EtWi00}, Etessami and Wilke define a modified version of the EF-games designed for LTL formulas. With these games, they show a strict hierarchy of expressive power of LTL formulas that classifies formulas based on their \textsf{Until} ($U$) depth. In this subsection, we define EF-games similar in spirit to their games.

As with all EF games, ours are played on two structures $\mathcal{M}$ and $\mathcal{M}'$ by two players called the spoiler and the duplicator. In our games, these structures are ordered trees. To simplify our analysis, we assume that each node in these trees is labelled by an alphabet from $\Sigma$, a finite set of alphabets. At the beginning of the game, a pebble is placed on some node in each structure. We refer to these nodes as the ``current'' nodes.

At the start of each round, the spoiler begins by picking a type of move. Each connective $\pi \in \{\Larrow, \Rarrow, \Uarrow, \Darrow\}$ corresponds to a type of move in this game. These moves have two stages. In the first stage, the spoiler picks one of the two structures, say $\mathcal{M}$, and places a second pebble at a node that can be reached from the current node in $\mathcal{M}$ by moving a few steps in the direction of $\pi$. For instance, if $\pi = \Larrow$, then the spoiler places a second pebble that's a left-sibling to the first node in $\mathcal{M}$. Importantly, if $\pi = \Darrow$, then the spoiler can pick any descendant of the current node. The duplicator responds by placing a second pebble in the other structure (which is $\mathcal{M}'$) that can be reached from its current node by moving in the same direction.

At this point, the spoiler can choose to end the move by removing the older pebbles from both structures. He can also choose to continue onto the second stage, which requires the spoiler to pick a node \emph{on the path connecting the two pebbles in the \emph{other structure}}, which is $\mathcal{M}'$. He places a third pebble at his selected node. The duplicator does the same in $\mathcal{M}$. The game proceeds by removing the older pebbles in both structures.

In our EF games, we limit the number of moves of a single type the spoiler can make. The sum of these limits forms the number of rounds of the game. Call an EF game a $(k_{\Larrow}, k_{\Rarrow}, k_{\Uarrow}, k_{\Darrow})$ game if the spoiler is allowed to make $k_\pi$ moves for each $\pi \in \{\Larrow, \Rarrow, \Uarrow, \Darrow\}$. For convenience, we often refer to the tuple using a vector notation, as in $\myvec{k} = (k_{\Larrow}, k_{\Rarrow}, k_{\Uarrow}, k_{\Darrow})$ and denote its sum $\sum k_\pi$ as $n$. Note that $n$ is the total number of rounds in the game.
...
\begin{definition}
    Let $\varphi$ be a $\mathcal{X}_{until}$ formula. For $\pi \in \{\Darrow, \Uarrow, \Larrow, \Rarrow\}$, the $\pi$ depth of $\varphi$ is simply the maximum number of $\pi$ connectives in a path from the root to a leaf in the formula tree of $\varphi$.
\end{definition}
The following lemma simplifies our arguments greatly. Note that the set of atomic propositions $\mathcal{P}$ is now $\Sigma$, with the usual caveat of only allowing one alphabet at a time.
\begin{lemma}
    \label{lemma:count-limiter-efgames}
    Suppose the set of all atomic propositions $\mathcal{P}$ was finite. Then, up-to expressive equivalence, there are finitely many $\mathcal{X}_{until}$ formulas of $\pi$ depth $\leq k_\pi$ for each $\pi \in \{\Darrow, \Uarrow, \Larrow, \Rarrow\}$ and for any tuple $\myvec{k} = (k_{\Larrow}, k_{\Rarrow}, k_{\Uarrow}, k_{\Darrow})$.
    \begin{remark*}
        For simplicity, we say that these formulas are \emph{depth-bound} by $\myvec{k}$.
    \end{remark*}
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    \proofsubparagraph{Case 1: No candidate formula differentiates the two structures.} In this case, we build the duplicator's winning strategy, by inducting on the number of rounds $n$.

    \begin{description}
        \item[Base case.] $n = 0$. Our assumptions ensure that the label of $t$ and $t'$ are identical, completing this case.
        \item[Induction step.]
            Suppose the spoiler plays a $\sigma$ move, and places a second pebble in $\mathcal{M}$ at a node $s$. Derive the tuple $\myvec{k'}$ from $\myvec{k}$ such that $k'_{\sigma} = k_{\sigma} - 1$ and $k'_\pi = k_\pi$ for all $\pi \neq \sigma$. Build the set $A$ of all (up-to equivalence) $\mathcal{X}_{until}$ formulas depth bound by $\myvec{k'}$ using \cref{lemma:count-limiter-efgames}.

            We now build the $\sigma$-formula that the spoiler has in mind. Let $\alpha$ be the conjunction of all formulas $\varphi \in A$ such that $\mathcal{M}, s \vDash \varphi$. This formula $\alpha$ becomes our target condition.

            For the path condition, we need to produce one that talks about all nodes on the path. Let $B$ be the set of all nodes in $\mathcal{M}$ on the path from $t$ to $s$. $B$ is necessarily finite. For each node $b \in B$, let $\psi_b$ be the conjunction of all formulas $\gamma \in A$ such that $\mathcal{M}, b \vDash \gamma$. Finally, let $\beta$ be the disjunction of all $\psi_b$. This produces our path condition.

            It's easy to see that $\alpha$ and $\beta$ are well-defined formulas depth bound by $\myvec{k'}$. Hence, the formula $\sigma(\alpha, \beta)$ is depth bound by $\myvec{k}$. This means that $\mathcal{M}', t' \vDash \sigma(\alpha, \beta)$. Thus, we are guaranteed a point $s' \in \mathcal{M}'$ with $\mathcal{M}', s' \vDash \alpha$ such that, for all nodes $b' \in \mathcal{M}'$ on the path from $t'$ to $s'$, $\mathcal{M}', b' \vDash \beta$.

            The duplicator picks this point $s' \in \mathcal{M}'$ and places his pebble there. If the spoiler continues the game from $s$ and $s'$, we employ the induction hypothesis to construct the duplicator's strategy. If the spoiler picks a point $b' \in \mathcal{M}'$ on the path from $t'$ to $s'$, the duplicator is aware that $\mathcal{M}', b' \vDash \beta$. Since $\beta$ is a disjunction of the $\psi_b$ formulas, $b'$ must satisfy one of them. Let $c \in \mathcal{M}$ be that node on the path from $t$ to $s$. Hence, $\mathcal{M}', b' \vDash \psi_c$ and $\mathcal{M}, c \vDash \psi_c$. The duplicator places his pebble at $c$, completing the round.

            At this stage, the game must proceed from $c$ and $b'$. Applying the induction hypothesis to this case finishes the proof.
    \end{description}

    \proofsubparagraph{Case 2: The formula $\varphi$ differentiates the two structures.} We similarly induct on $n$.
    \begin{description}
        \item[Base case.] $n = 0$. The assumptions force different labels on $t$ and $t'$, forcing a spoiler victory.
        \item[Induction step.]
            Let $A$ be the set of all top-level $\{\Larrow, \Rarrow, \Uarrow, \Darrow\}$ subformulas in $\varphi$. Clearly, if $\mathcal{M}, t \vDash \psi$ and $\mathcal{M}', t' \vDash \psi$ for every $\psi \in B$, both structures model $\varphi$. Hence, there must be one subformula $\sigma(\alpha, \beta)$ for $\sigma \in \{\Larrow, \Rarrow, \Uarrow, \Darrow\}$ such that, without loss of generality,
            \begin{equation*}
                \mathcal{M}, t \vDash \sigma(\alpha, \beta) \land \mathcal{M}', t' \nvDash \sigma(\alpha, \beta)
            \end{equation*}
            This means that there is a point $s \in \mathcal{M}$ that can be reached by moving in the direction of $\sigma$ such that $\mathcal{M}, s \vDash \alpha$ and for all points $r$ in the path from $t$ to $s$, $\mathcal{M}, r \vDash \beta$. The spoiler places his pebble on this point $s$.

            If the duplicator picks a point $s' \in \mathcal{M}'$ such that $\mathcal{M}', s' \nvDash \alpha$, the spoiler continues the game from $s$ and $s'$. At this stage, we can apply the induction hypothesis to produce the spoiler's strategy. Otherwise, by construction, there must be some point $r' \in \mathcal{M}'$ on the path from $t'$ to $s'$ such that $\mathcal{M}', r' \nvDash \beta$. The spoiler picks this point $r' \in \mathcal{M}'$. No matter which point $r$ the duplicator picks, we will have that $\mathcal{M}, r \vDash \beta$. This allows us to use the induction hypothesis at the game starting from $r$ and $r'$, completing the proof.
    \end{description}
\end{proof}
